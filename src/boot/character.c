...
//For mario characters, double their size
void Character_DrawParallax2(Character *this, Gfx_Tex *tex, const CharFrame *cframe, fixed_t parallax)
{
	//Draw character
	fixed_t x = this->x - FIXED_MUL(stage.camera.x, parallax) - FIXED_DEC(cframe->off[0],1);
	fixed_t y = this->y - FIXED_MUL(stage.camera.y, parallax) - FIXED_DEC(cframe->off[1],1);
...
void Character_Draw2(Character *this, Gfx_Tex *tex, const CharFrame *cframe)
{
	Character_DrawParallax2(this, tex, cframe, FIXED_UNIT);
}
